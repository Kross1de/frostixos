...
kernel_status_t vbe_init(void) {
    ...
    memcpy(&g_device.control_info, (void *)mbi->vbe_control_info, sizeof(vbe_control_info_t));
    memcpy(&g_device.mode_info, (void *)mbi->vbe_mode_info, sizeof(vbe_mode_info_t));
...
u8* vbe_get_framebuffer(void) {
    return (u8 *)g_device.framebuffer_addr;
}
...
u32 vbe_color_to_pixel(vbe_color_t color) {
    ...
    u32 a = (u32)color.alpha * ((1 << g_device.mode_info.rsvd_mask_size) - 1) / 255;
...
vbe_color_t vbe_pixel_to_color(u32 pixel) {
    ...
    color.green = ((pixel >> g_device.green_field_position) & g_mask) * 255 / g_mask;
    color.blue = ((pixel >> g_device.blue_field_position) & b_mask) * 255 / b_mask;
    color.alpha = ((pixel >> g_device.mode_info.rsvd_field_position) & a_mask) * 255 / a_mask;
