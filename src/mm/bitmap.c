...
static void mark_region_allocated(u64 start, u64 len) {
    ...
    for (u32 page = start_page; page < end_page && page < g_physical_allocator.total_pages; page++) {
        bitmap_set_bit(page);
...
kernel_status_t pmm_init(multiboot_info_t* mb_info) {
    ...
    g_physical_allocator.size = ALIGN_UP(g_physical_allocator.total_pages / 8, PAGE_SIZE);
