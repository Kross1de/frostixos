...
static void rtc_read(time_t *t)
{
	...
	if (!is_binary && century)
		century = bcd_to_bin(century);

	t->year += (century ? century : 20) * 100;
}
...
void draw_status(void)
{
	...
	/* Clear background for the status area and draw text */
...
void time_update(void)
{
	...
	if (g_ticks - last_second_tick >= pit_frequency) {
		...
		/* Simple second increment, consult RTC on day rollover */
...
{
	return (g_ticks * 1000) / pit_frequency;
}
