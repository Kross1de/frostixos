 ...
 *
 * Cleanups:
 *  - Minor bounds checks and explicit casts.
 */
...
static inline size_t vga_index(u16 x, u16 y)
{
	return (size_t)y * VGA_TEXT_WIDTH + x;
}
...
static void vga_text_scroll_internal(void)
{
	for (u16 y = 0; y < VGA_TEXT_HEIGHT - 1; y++) {
		for (u16 x = 0; x < VGA_TEXT_WIDTH; x++) {
			...
			size_t dst_index = vga_index(x, y);
			g_vga_text_device.buffer[dst_index] = g_vga_text_device.buffer[src_index];
		}
	}
	for (u16 x = 0; x < VGA_TEXT_WIDTH; x++) {
		size_t idx = vga_index(x, VGA_TEXT_HEIGHT - 1);
		g_vga_text_device.buffer[idx] = vga_make_entry(' ', g_vga_text_device.color);
	}
}
...
kernel_status_t vga_text_init(void)
{
	...
	g_vga_text_device.color = vga_text_make_color(VGA_TEXT_COLOR_LIGHT_GREY, VGA_TEXT_COLOR_BLACK);
...
{
	return g_vga_text_device.cursor;
}
...
kernel_status_t vga_text_move_cursor(s16 dx, s16 dy)
{
	...
	if (new_x < 0) new_x = 0;
	if (new_x >= VGA_TEXT_WIDTH) new_x = VGA_TEXT_WIDTH - 1;
	if (new_y < 0) new_y = 0;
	if (new_y >= VGA_TEXT_HEIGHT) new_y = VGA_TEXT_HEIGHT - 1;
...
kernel_status_t vga_text_putchar_at(char c, u8 color, u16 x, u16 y)
{
	...
	if (x >= VGA_TEXT_WIDTH || y >= VGA_TEXT_HEIGHT)
		return KERNEL_INVALID_PARAM;
	size_t idx = vga_index(x, y);
	g_vga_text_device.buffer[idx] = vga_make_entry(c, color);
	return KERNEL_OK;
}
...
kernel_status_t vga_text_putchar(char c)
{
	...
	vga_text_putchar_at(c, g_vga_text_device.color,
			    g_vga_text_device.cursor.x,
			    g_vga_text_device.cursor.y);
...
kernel_status_t vga_text_write(const char *data, size_t size)
{
	...
	for (size_t i = 0; i < size; i++) {
		if (vga_text_putchar(data[i]) != KERNEL_OK)
			return KERNEL_ERROR;
	}
	return KERNEL_OK;
}
