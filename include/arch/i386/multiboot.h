...
/*
 * Multiboot header and information structures used to communicate with
 * Multiboot-compliant bootloaders (e.g., GRUB). All structures are packed
 * to match bootloader-supplied memory layouts.
 */
...
/* Multiboot header present in the kernel image */
typedef struct {
	u32 magic;
	u32 flags;
	u32 checksum;
...
/* Module descriptor passed by bootloader */
typedef struct {
	u32 mod_start;
	u32 mod_end;
	u32 string;
...
/* VBE info structure as carried by multiboot */
typedef struct {
	char signature[4];
	u16 version;
	u32 oem_string_ptr;
...
/* VBE mode info passed by bootloader */
typedef struct {
	u16 mode_attributes;
	u8 win_a_attributes;
	u8 win_b_attributes;
...
/* Multiboot information structure supplied by the bootloader */
typedef struct {
	u32 flags;
	u32 mem_lower;
	u32 mem_upper;
